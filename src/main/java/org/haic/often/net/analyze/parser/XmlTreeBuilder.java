 ...
 * document.
 * <p>Usage example: {@code Document xmlDoc = Parse.parse(html, baseUrl, Parser.xmlParser());}</p>
 *
...
public class XmlTreeBuilder extends TreeBuilder {
	...
	protected void initialiseParse(Reader input, String baseUri, Parser parser) {
		...
		doc.outputSettings().syntax(Document.OutputSettings.Syntax.xml).escapeMode(Entities.EscapeMode.xhtml).prettyPrint(false); // as XML, we don't understand what whitespace is significant or not
	}
	...
	Element insert(Token.StartTag startTag) {
		assert startTag.attributes != null;
		Tag tag = tagFor(startTag.name(), settings);
		//  wonder if for xml parsing, should treat all tags as unknown? because it's not html.
