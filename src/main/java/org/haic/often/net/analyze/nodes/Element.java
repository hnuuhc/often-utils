...
import org.haic.often.net.analyze.select.*;
...
public class Element extends Node {
	...
	public String baseUri() {
		return searchUpForAttribute(this);
	}

	private static String searchUpForAttribute(final Element start) {
		Element el = start;
		while (el != null) {
			if (el.attributes != null && el.attributes.hasKey(Element.BaseUriKey)) return el.attributes.get(Element.BaseUriKey);
			el = el.parent();
		}
		return "";
	}
	 ...
	 *
	 * @param key   name of the attribute
	 * @param regex regular expression to match against attribute values. You can use <a href="http://java.sun.com/docs/books/tutorial/essential/regex/pattern.html#embedded">embedded flags</a> (such
	 *              as (?i) and (?m) to control regex options.
	 * @return elements that have attributes matching this regular expression
	 */
	 ...
	 * Find elements whose text matches the supplied regular expression.
	 *
	 * @param regex regular expression to match text against. You can use <a href="http://java.sun.com/docs/books/tutorial/essential/regex/pattern.html#embedded">embedded flags</a> (such as (?i) and
	 *              (?m) to control regex options.
	 * @return elements matching the supplied regular expression.
	 * @see Element#text()
	 */
		...
		NodeTraversor.traverse(new NodeVisitor() {
			public void head(Node node, int depth) {
				if (node instanceof TextNode textNode) {
					...
					if (accum.length() > 0 && (element.isBlock() || element.tag.normalName().equals("br")) && !TextNode.lastCharIsWhitespace(accum)) accum.append(' ');
				}
			}
			...
			public void tail(Node node, int depth) {
				...
				if (node instanceof Element element) {
					if (element.isBlock() && (node.nextSibling() instanceof TextNode) && !TextNode.lastCharIsWhitespace(accum)) accum.append(' ');
				}
	...
	public Element text(String text) {
		...
		if (owner != null && owner.parser().isContentForTagData(normalName())) appendChild(new DataNode(text));
	...
	void outerHtmlTail(Appendable accum, int depth, Document.OutputSettings out) throws IOException {
		if (!(childNodes.isEmpty() && tag.isSelfClosing())) {
			if (out.prettyPrint() && (!childNodes.isEmpty() && (tag.formatAsBlock() || (out.outline() && (childNodes.size() > 1 || (childNodes.size() == 1 && (childNodes.get(0) instanceof Element)))))))
				indent(accum, depth, out);
