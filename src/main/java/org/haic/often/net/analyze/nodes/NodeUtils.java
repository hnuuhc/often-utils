...
import org.haic.often.net.analyze.helper.W3CDom;
import org.haic.often.net.analyze.parser.HtmlTreeBuilder;
...
/**
 * Internal helpers for Nodes, to keep the actual node APIs relatively clean. internal class, so don't use it as
...
final class NodeUtils {
	 ...
	 * from the original element. The original elements are then fetched from the w3c doc user data (where we
	 * stashed them during conversion). This process could potentially be optimized by transpiling the compiled xpath
	 * expression to  Evaluator when there's 1:1 support, thus saving the W3C document conversion stage.
	 */
