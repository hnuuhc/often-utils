...
import static org.haic.often.net.analyze.parser.HtmlTreeBuilderState.Constants.InTableFoster;
...
public class HtmlTreeBuilder extends TreeBuilder {
	...
	List<Node> parseFragment(String inputFragment, @Nullable Element context, String baseUri, Parser parser) {
		...
		if (context != null) {
			if (context.ownerDocument() != null) // quirks setup:
				doc.quirksMode(Objects.requireNonNull(context.ownerDocument()).quirksMode());

			// initialise the tokeniser state:
			String contextTag = context.normalName();
			switch (contextTag) {
				case "title", "textarea" -> tokeniser.transition(TokeniserState.Rcdata);
				case "iframe", "noembed", "noframes", "style", "xml" -> tokeniser.transition(TokeniserState.Rawtext);
	...
	void error(HtmlTreeBuilderState state) {
		if (parser.getErrors().canAddError()) parser.getErrors().add(new ParseError(reader, "Unexpected %s token [%s] when in state [%s]", currentToken.tokenType(), currentToken, state));
	}

	Element insert(final Token.StartTag startTag) {
		// cleanup duplicate attributes:
		if (startTag.hasAttributes() && !Objects.requireNonNull(startTag.attributes).isEmpty()) {
	...
	Element insertEmpty(Token.StartTag startTag) {
		...
		if (startTag.isSelfClosing()) {
			if (tag.isKnownTag()) {
				if (!tag.isEmpty()) tokeniser.error("Tag [%s] cannot be self closing; not a void tag", tag.normalName());
		...
		for (int pos = bottom; pos >= upper; pos--) {
			...
			switch (name) {
				case "select":
					...
					// should loop up (with some limit) and check for table or template hits
					break LOOP;
				case "td":
				case "th":
					if (!last) {
	...
	void setHeadElement(@Nullable Element headElement) {
	 ...
	 * When the steps below require the UA to generate implied end tags, then, while the current node is a dd element, a dt element, an li element, an optgroup element, an option element, a p element,
	 * an rb element, an rp element, an rt element, or an rtc element, the UA must pop the current node off the stack of open elements.
	 * <p>
	 * If a step requires the UA to generate implied end tags but lists an element to exclude from the process, then the UA must perform the above steps as if that element was not in the above list.
	 * <p>
	 * When the steps below require the UA to generate all implied end tags thoroughly, then, while the current node is a caption element, a colgroup element, a dd element, a dt element, an li
	 * element, an optgroup element, an option element, a p element, an rb element, an rp element, an rt element, an rtc element, a tbody element, a td element, a tfoot element, a th element, a thead
	 * element, or a tr element, the UA must pop the current node off the stack of open elements.
	 *
	...
	void insertInFosterParent(Node in) {
		...
		if (isLastTableParent) {
			...
			lastTable.before(in);
		} else Objects.requireNonNull(fosterParent).appendChild(in);
	}
