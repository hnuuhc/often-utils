...
public class TypeUtil {
    ...
    public static <T> List<T> convertList(Object obj, @NotNull List<T> list, @NotNull Class<T> itemClass) {
        ...
        switch (obj) {
            ...
            case Collection<?> c -> objs = c.toArray();
            case Object[] os -> objs = os;
            case int[] os -> objs = Arrays.stream(os).boxed().toArray();
            case long[] os -> objs = Arrays.stream(os).boxed().toArray();
            case double[] os -> objs = Arrays.stream(os).boxed().toArray();
            case float[] os -> {
                var oss = new ArrayList<>();
                for (var e : os) oss.add(e);
                objs = oss.toArray();
            }
            case short[] os -> {
                var oss = new ArrayList<>();
            ...
            default -> JSONArray.parseArray(String.valueOf(obj)).toArray();
        }
        assert objs != null;
